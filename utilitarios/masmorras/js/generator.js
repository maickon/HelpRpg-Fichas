...
function generate_text(a) {
    if (a = gen_data[a]) if (a = select_from(a)) {
        var c = new_trace();
...
function expand_token(a, c) {
    ...
    else if (b = /^alt (.+) def (.+)/.exec(a)) return (d = gen_data[b[1]]) ? expand_tokens(select_from(d), c) : (d = gen_data[b[2]]) ? expand_tokens(select_from(d), c) : b[2];
    ...
    else if (b = /^get (\w+) fix (.+)/.exec(a)) return c["var"][b[1]] || set_var(b[1], b[2], c);
