...
(function () {
    ...
    function keys(object) {
        ...
        if (IS_DONTENUM_BUGGY) for (var i = 0; property = DONT_ENUMS[i]; i++) if (_hasOwnProperty.call(object, property)) results.push(property);
        return results
    }
...
Object.extend(String.prototype, function () {
    ...
    function evalJSON(sanitize) {
        var json = this.unfilterJSON(),
            cx = /[\u0000\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g;
        if (cx.test(json)) json = json.replace(cx, function (a) {
...
(function () {
    ...
    function filter(iterator) {
        ...
        var results = [],
            context = arguments[1],
            value;
        for (var i = 0, length = object.length >>> 0; i < length; i++) if (i in object) {
            value = object[i];
...
Ajax.Updater = Class.create(Ajax.Request, {
    ...
    updateContent: function (responseText) {
        ...
        if (receiver = $(receiver)) if (options.insertion) if (Object.isString(options.insertion)) {
            var insertion = {};
...
(function (GLOBAL) {
    ...
    if (!GLOBAL.Node.ELEMENT_NODE) Object.extend(GLOBAL.Node, {
        ELEMENT_NODE: 1,
        ATTRIBUTE_NODE: 2,
        TEXT_NODE: 3,
    ...
    function update(element, content) {
        ...
        if (ANY_INNERHTML_BUGGY) if (tagName in INSERTION_TRANSLATIONS.tags) {
    ...
    function adjacent(element) {
        ...
        var siblings = Element.siblings(element),
            results = [];
        for (var i = 0, sibling; sibling = siblings[i]; i++) if (Prototype.Selector.match(sibling, expressions)) results.push(sibling);
        return results
    }
    ...
    function hasAttribute_IE(element, attribute) {
        if (attribute === "checked") return element.checked;
    ...
    var _getEv;
    if (String(onclickValue).indexOf("{") > -1) _getEv = function (element, attribute) {
    ...
    else if (onclickValue === "") _getEv = function (element, attribute) {
        var value = element.getAttribute(attribute);
        if (!value) return null;
        return value.strip()
    };
    ...
    function addMethods(methods) {
        ...
        else if (Object.isArray(tagName)) for (var i = 0, tag; tag = tagName[i]; i++) addMethodsToTagName(tag, methods);
    ...
    else GLOBAL.Element.extend.refresh = function () {
...
(function () {
    ...
    function getPixelValue(value, property, context) {
        ...
        if (element && isPercentage) {
            ...
            if (context === document.viewport) if (isHorizontal) whole = document.viewport.getWidth();
                ...
                if (isVertical) whole = document.viewport.getHeight()
    ...
    if ("currentStyle" in document.documentElement) hasLayout = function (element) {
    ...
    if ("getBoundingClientRect" in document.documentElement) Object.extend(Element.Layout.COMPUTATIONS, {
        right: function (element) {
    ...
    } else if (Prototype.Browser.Webkit) cumulativeOffset = function (element) {
        element = $(element);
        var valueT = 0,
            valueL = 0;
        do {
    ...
    if ("getBoundingClientRect" in document.documentElement) Element.addMethods({
...
Form.Methods = {
    ...
    getElements: function (form) {
        ...
        for (var i = 0; element = elements[i]; i++) if (serializers[element.tagName.toLowerCase()]) results.push(Element.extend(element));
        return results
    },
...
(function (GLOBAL) {
    ...
    if (window.attachEvent) if (window.addEventListener) isIELegacyEvent = function (event) {
        return !(event instanceof
        window.Event)
    };
    else isIELegacyEvent = function (event) {
        return true
    };
    var _isButton;
    ...
    function unregister(element, eventName, handler) {
        ...
        var i = entries.length,
            entry;
        while (i--) if (entries[i].handler === handler) {
            entry = entries[i];
            break
        }
        if (!entry) return;
...
if (!document.getElementsByClassName) document.getElementsByClassName = function (instanceMethods) {
    ...
    function (element, className) {
        ...
        className = " " + className + " ";
        for (var i = 0, child, cn; child = nodes[i]; i++) if (child.className && (cn = " " + child.className + " ") && (cn.include(className) || classNames && classNames.all(function (name) {
