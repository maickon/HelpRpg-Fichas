	...
	$tag->div(['class'=>'row']);
		$tag->div(['class'=>'col-md-8']);
		    ...
		    $tag->span(['class'=>'help-inline']);
		        $tag->input(['class'=>'btn btn-success margin', 'type'=>'button', 'value'=>'Gerar Taverna', 'onclick'=>'rand_aventura_apocaliptica();']);
		    $tag->spam;
		...
		$tag->div(['class'=>'col-md-12', 'id'=>'content']);
			...
			$tag->div('class="title hide-title"');
				$tag->printer("Idéias para aventuras com tema apocaliptico.");
			$tag->div;
		     $tag->div('class="row"');
