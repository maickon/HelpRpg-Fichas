...
function helper_show_monstro_ded4_0($personagem){
	...
	$tag->div('class="col-md-12 body2_ded_4-0"');
		$form->_row();
			$form->_col(4);
				$tag->imprime($form->bold(FORCA)." {$unserialize_params['forca']}");
			$form->col_();
			$form->_col(4);
				$tag->imprime($form->bold(CON)." {$unserialize_params['constituicao']}");
			$form->col_();
			$form->_col(4);
				$tag->imprime($form->bold(DES)." {$unserialize_params['destreza']}");
			$form->col_();
			$form->_col(4);
				$tag->imprime($form->bold(INTELIGENCIA)." {$unserialize_params['inteligencia']}");
			$form->col_();
			$form->_col(4);
				$tag->imprime($form->bold(SAB)." {$unserialize_params['sabedoria']}");
			$form->col_();
			$form->_col(4);
				$tag->imprime($form->bold(CAR)." {$unserialize_params['carisma']}");
			$form->col_();
		$form->row_();
	$tag->div;
