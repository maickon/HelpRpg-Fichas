...
$tag->br();
	...
	$objeto = new Personagens(ROOTPATHURL.MONSTROIMGPATH);
	$monstro = $objeto->select($objeto->getTable(), null, [ ['tipo', '=', 'Monstro'] ]);
	...
	$form->_container();
		...
		$form->_col(2);
			if($s->get_session('nome')):
				helper_new_or_register_button(ROOTPATHURL.MONSTROPATH.'new.php', NOVO);
			else:
			...
			$tag->tbody();
				foreach($monstro as $a):
					$tag->tr();
						...
						if($super):
							helper_componentes_buttons('monstros', $a['id']);
						elseif($s->get_session('nome') != $a['dono']):
							...
							helper_componentes_buttons('monstros', $a['id'], $off = true);
						elseif($s->get_session('nome') == $a['dono']):
