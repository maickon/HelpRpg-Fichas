...
$tag->br();
	...
	$objeto = new Contos();
	$contos = $objeto->select($objeto->getTable());
	...
	$form->_container();
		...
		$form->_col(2);
			if($s->get_session('nome')):
				helper_new_or_register_button(ROOTPATHURL.CONTOSPATH.'new.php', NOVO);
			else:
			...
			$tag->tbody();
				foreach($contos as $a):
					$tag->tr();
						...
						if($super):
							helper_componentes_buttons('contos', $a['id']);
						elseif($s->get_session('contos') != $a['dono']):
							//em cada linha onde o usuario atual for diferente do dono da arma, vai neutralizar as opçoes de editar e excluir 
							helper_componentes_buttons('contos', $a['id'], $off = true);
						elseif($s->get_session('contos') == $a['dono']):
