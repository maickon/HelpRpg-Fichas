...
$tag->br();
	...
	$objeto = new Talentos(ROOTPATHURL.TALENTOSIMGPATH);
	...
	$form->_container();
		...
		$form->_col(2);
			if($s->get_session('nome')):
				helper_new_or_register_button(ROOTPATHURL.TALENTOSPATH.'new.php', NOVO);
			else:
			...
			$tag->tbody();
				foreach($personagens as $a):
					$tag->tr();
						...
						if($super):
							helper_componentes_buttons('talentos', $a['id']);
						elseif($s->get_session('nome') != $a['dono']):
							...
							helper_componentes_buttons('talentos', $a['id'], $off = true);
						elseif($s->get_session('nome') == $a['dono']):
