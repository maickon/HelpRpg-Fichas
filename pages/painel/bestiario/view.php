...
$tag->br();
	...
	$form->_container();
		...
		if($super):
			helper_componentes_buttons_view('bestiario', $contos[0]['id']);
		elseif($s->get_session('nome') != $contos[0]['dono']):
			...
			helper_componentes_buttons_view('bestiario', $contos[0]['id'], $off = true);
		elseif($s->get_session('nome') == $contos[0]['dono']):
		...
		$form->_container();
			$form->_col(12);
				$form->h1($contos[0]['nome']);
				$tag->b();
				$tag->imprime(CRIADO_NO_DIA);
				echo date('d/m/Y', strtotime($contos[0]['criado_em'])).' às '.date(' H:i:s', strtotime($contos[0]['criado_em']));
				$tag->imprime(CADASTRADO_POR." {$contos[0]['dono']} | ".CLASSIFICACAO.": {$contos[0]['classificacao']}");
				$tag->b;
				$tag->br();
				$tag->br();
				$tag->imprime($contos[0]['descricao']);
			$form->col_();	
		$form->container_();
	$form->div_();
