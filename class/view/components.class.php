				...
				$tag->div('class="panel panel-default"');
					...
					$tag->form('class="form-signin" action="'.LOGIN_VALIDATION_PATH.'" method="post"');
						$tag->h4('class="panel-title"');
							$tag->imprime('Preencha os campos abaixo e faça o login.');
						$tag->h4;
						$tag->br();
						...
						$tag->input('type="text" id="inputLogin" name="login" class="form-control input-login" placeholder="E-mail" required autofocus');
						$tag->label('for="inputPassword" class="sr-only"');
							$tag->imprime('Password');
						$tag->label;
						$tag->br();
						$tag->input('type="password" id="inputPassword" name="senha" class="form-control input-login" placeholder="Senha" required');
						...
						$tag->label;
						$tag->div;
						$tag->input('name="entrar" value="Entrar" class="btn btn-lg btn-primary btn-block button-login" type="submit"');
					$tag->form;
				$tag->div;
			$tag->div;
		$tag->div;
